...
Over-ride the operating system detection logic and force the UNIX
mode.  See also B<--windows>.

=item B<--windows>

Over-ride the operating system detection logic and force the 
Microsoft Windows mode.  See also B<--unix>.
