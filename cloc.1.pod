...
archives.  Use --diff-alignment to generate a list showing
which file pairs where compared.  See also --ignore-case, --ignore-whitespace.
...
when comparing files with --diff.  See also --ignore-whitespace.
...
Count stdin as if it had the name <file>.
